...
public:
  HartAlgorithmModified(vector<pair<int, vector<double> > > train_objects)
  {
    ...
    // changes here! ------------------------------------------------------------------------------------------
    double tmp_dist;
    int neightbors_counter;
    vector<pair<int, pair<int, vector<double> > > > tmp_ordernation;
    for (const pair<int, vector<double> > p : std_train_objects)
    {
      // look for the nearest with different class
      for (const pair<int, vector<double> > q : std_train_objects)
      {
        if( p.first != q.first)
        {
          tmp_dist = dist(p.second, q.second);
        }
        break;
      }
      // look how many of your same class is in the circle with radius tmp_dist
      neightbors_counter=0;
      for (const pair<int, vector<double> > q : std_train_objects)
      {
        if( p.first == q.first and dist(p.second, q.second) < tmp_dist)
        {
          neightbors_counter++;
        }
      }
      tmp_ordernation.push_back( make_pair(neightbors_counter, p) );
    }
    sort( tmp_ordernation.begin(), tmp_ordernation.end(),
      [](const pair<int, pair<int, vector<double> > > &p1, const pair<int, pair<int, vector<double> > > &p2)
      {
        return p1.first < p2.first;
    });
    std_train_objects.clear();
    for(auto &a : tmp_ordernation)
    {
...
int main (int argc, char ** argv) 
{
  ...
  HartAlgorithmModified myClassifier(train_objects);
